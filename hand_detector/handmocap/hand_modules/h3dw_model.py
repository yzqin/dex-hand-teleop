...
import hand_detector.mocap_utils.general_utils as gnu
# dct is the abbr. of Human Model recovery with Densepose supervision
import numpy as np
import smplx
import torch
from hand_detector.handmocap.hand_modules.h3dw_networks import H3DWEncoder
...
class H3DWModel(object):
    ...
    def __init__(self, opt):
        ...
        self.smplx: smplx.SMPLX = smplx.create(
            smplx_model_path,
            model_type="smplx",
    ...
    def forward(self):
        ...
        self.pred_pose_params = self.final_params[:, cam_dim: (cam_dim + pose_dim)]
        self.pred_shape_params = self.final_params[:, (cam_dim + pose_dim):]

        #  get predicted smpl verts and joints,
        self.pred_verts, self.pred_joints_3d = self.get_smplx_output(self.pred_pose_params, self.pred_shape_params)
